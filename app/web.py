...
async def health():
    ...
    news_stats = _service.news.status() if hasattr(_service, "news") else {}
    return {
        "status": "ok",
        ...
        "market_monitorable": _service.market_is_monitorable(),
        "storage_backend": _service.store.__class__.__name__,
        ...
        "intraday_enabled": bool(getattr(_service.s, "intraday_enabled", True)),
        "multi_session_enabled": bool(getattr(_service.s, "multi_session_enabled", True)),
        "news_enabled": bool(news_stats.get("enabled", False)),
        "news_source_state": news_stats.get("source_state"),
        "news_cached_items": news_stats.get("cached_items", 0),
