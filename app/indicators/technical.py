...
def add_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Price structure and retest diagnostics for the Hunter/Judge pipeline.
    prev_high5 = x["high"].rolling(5, min_periods=5).max().shift(1)
    prev_low5 = x["low"].rolling(5, min_periods=5).min().shift(1)
    older_high5 = x["high"].rolling(5, min_periods=5).max().shift(6)
    older_low5 = x["low"].rolling(5, min_periods=5).min().shift(6)
    hh = prev_high5 > older_high5
    hl = prev_low5 > older_low5
    lh = prev_high5 < older_high5
    ll = prev_low5 < older_low5
    x["structure_hh"] = hh.astype(float)
    x["structure_hl"] = hl.astype(float)
    x["structure_lh"] = lh.astype(float)
    x["structure_ll"] = ll.astype(float)
    x["structure_bullish"] = (hh & hl).astype(float)
    x["structure_bearish"] = (lh & ll).astype(float)
    # A retest is only confirmed when the prior resistance is revisited intrabar
    # and the candle closes back above it.
    retest_tol = x["atr14"] * 0.12
    x["retest_confirmed"] = (
        (x["low"] <= (x["resistance20"] + retest_tol))
        & (x["close"] > x["resistance20"])
        & (x["close_position"] >= 0.55)
    ).astype(float)
...
def latest_features(df: pd.DataFrame) -> dict:
    ...
    keys = [
        ...
        "is_breakout", "failed_breakout", "structure_hh", "structure_hl", "structure_lh", "structure_ll",
        "structure_bullish", "structure_bearish", "retest_confirmed", "bb_mid20", "bb_upper20", "bb_lower20", "bb_position",
    ]
    ...
    for k in keys:
        ...
        if pd.notna(value) and np.isfinite(float(value)):
            out[k] = float(value)
    if out.get("structure_bullish", 0) >= 0.5:
        out["structure_state"] = "HH_HL"
    elif out.get("structure_bearish", 0) >= 0.5:
        out["structure_state"] = "LH_LL"
    else:
        out["structure_state"] = "MIXED"
    return out
