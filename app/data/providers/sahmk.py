...
class SahmkProvider(DataProvider):
    ...
    def __init__(
        ...
        self._daily_request_count = 0  # outbound attempts, including failed/retried requests
    ...
    async def _get(self, path, params=None):
        ...
        while True:
            ...
            # Count every outbound attempt BEFORE the network call. Providers may
            # charge failed/retried requests against the daily quota too.
    ...
    def stats(self):
        ...
        return {
            ...
            "local_attempted_requests": self._daily_request_count,
            "local_successful_requests": self._daily_successful_requests,
    ...
    async def top_value(self, limit=50, index="TASI"):
        """Return most-active TASI names ranked by traded value (Free endpoint)."""
        limit = max(1, min(int(limit), 100))
        payload = await self._get(
            "/market/value/",
            {"limit": limit, "index": index, "data_mode": "delayed"},
    ...
    async def top_value_quotes(self, limit=50, index="TASI"):
        rows = await self.top_value(limit=limit, index=index)
        quotes = []
        for row in rows:
            quote = self.quote_from_payload(row)
            if quote is not None:
    ...
    async def top_gainers(self, limit=50, index="TASI"):
        """Return top gaining stocks by percentage change (Free SAHMK endpoint)."""
        limit = max(1, min(int(limit), 100))
        payload = await self._get(
            "/market/gainers/",
            {"limit": limit, "index": index, "data_mode": "delayed"},
        ...
        if isinstance(payload, dict):
            rows = payload.get("gainers", payload.get("stocks", payload.get("results", payload.get("data", []))))
        elif isinstance(payload, list):
            rows = payload
        else:
            rows = []
    ...
    async def top_gainers_quotes(self, limit=50, index="TASI"):
        rows = await self.top_gainers(limit=limit, index=index)
        quotes = []
        for row in rows:
            quote = self.quote_from_payload(row)
            if quote is not None:
    ...
    async def market_summary(self):
        payload = await self._get("/market/summary/")
        if not isinstance(payload, dict):
            return payload
        data = payload.get("data") if isinstance(payload.get("data"), dict) else payload
        return {
            "index_value": data.get("index_value", data.get("value", data.get("tasi_value"))),
            "change_percent": data.get("index_change_percent", data.get("change_percent", data.get("change_pct"))),
            "advancers": data.get("advancing", data.get("advancers")),
            "decliners": data.get("declining", data.get("decliners")),
            "total_volume": data.get("total_volume", data.get("volume", data.get("market_volume"))),
            "trading_value": data.get("trading_value", data.get("value_traded", data.get("total_value"))),
            "raw": data,
        }
