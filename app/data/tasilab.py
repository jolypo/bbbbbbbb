...
class TasilabProvider(DataProvider):
    ...
    def __init__(self, settings):
        ...
        self._daily_successful_requests = 0
        self._daily_request_attempts = 0
        self._daily_request_date = datetime.now(ZoneInfo(getattr(settings, "timezone", "Asia/Riyadh"))).date()
        ...
        # Lightweight market/status health is tracked independently from the
        # single-quote circuit. A burst of quote 5xx errors must not hide a
        # healthy market/status endpoint that can still provide TASI breadth.
        self._market_status_cache = None
        self._market_status_cache_at = 0.0
        self._market_status_last_success = None
        self._market_status_last_error = None
    ...
    def _reset_daily_counter_if_needed(self):
        today = datetime.now(ZoneInfo(getattr(self.s, "timezone", "Asia/Riyadh"))).date()
    ...
    async def _throttle(self, *, ignore_single_quote_circuit=False):
        async with self._request_lock:
            ...
            if (not ignore_single_quote_circuit) and now < self._circuit_until:
    ...
    async def _get(
        ...
        params=None,
        *,
        ignore_single_quote_circuit=False,
    ):
        last_exc = None

        for attempt in range(self.max_retries + 1):
            await self._throttle(ignore_single_quote_circuit=ignore_single_quote_circuit)
            ...
            self._daily_request_attempts += 1
            self._recent_request_times.append(time.monotonic())
            cutoff = time.monotonic() - 60.0
            self._recent_request_times = [ts for ts in self._recent_request_times if ts >= cutoff][-500:]
            try:
                ...
                self._request_log.append({
                    "time": datetime.now(ZoneInfo(getattr(self.s, "timezone", "Asia/Riyadh"))).strftime("%H:%M:%S"),
                    "path": path,
                    "status": response.status_code,
                    "caller": caller,
                })
                self._request_log = self._request_log[-20:]
            ...
            self._reset_daily_counter_if_needed()
            self._daily_successful_requests += 1
            self._cooldown_until = 0.0
    ...
    def quote_from_payload(self, data, fallback_symbol=None, fallback_timestamp=None):
        ...
        # Never substitute HTTP fetch time for market-data time. A response can
        # arrive now while containing an old cached snapshot. Missing market
        # timestamps therefore remain None and are rejected by freshness gates.
        if updated_at is None:
            raw["_timestamp_source"] = "missing"
    ...
    async def market_status(self, force=False):
        """Return Tasilab market status through the lightest documented endpoint.

        The endpoint is allowed to run while the single-quote circuit is open.
        Quote 502s and market-status health are separate failure domains. A
        short cache avoids repeatedly spending requests on the same delayed
        market snapshot.
        """
        ttl = max(10, int(getattr(self.s, "tasilab_market_status_cache_seconds", 60) or 60))
        now = time.monotonic()
        if (
            not force
            and self._market_status_cache is not None
            and (now - self._market_status_cache_at) < ttl
        ):
            return self._market_status_cache
        try:
            payload = await self._get(
                "/v1/market/status",
                ignore_single_quote_circuit=True,
            )
            self._market_status_cache = payload
            self._market_status_cache_at = time.monotonic()
            self._market_status_last_success = datetime.now(
                ZoneInfo(getattr(self.s, "timezone", "Asia/Riyadh"))
            ).strftime("%H:%M:%S")
            self._market_status_last_error = None
            return payload
        except Exception as exc:
            self._market_status_last_error = str(exc)[:240]
            raise

    async def market_is_open(self):
        payload = await self.market_status()
        data = self._extract_dict(payload)
        for key in ("is_open", "market_open", "open"):
            if key in data:
                value = data.get(key)
                if isinstance(value, bool):
                    return value
                text = str(value).strip().lower()
                if text in {"1", "true", "yes", "open", "opened"}: return True
                if text in {"0", "false", "no", "closed", "close"}: return False
        status = str(data.get("status", data.get("market_status", ""))).strip().lower()
        if status in {"open", "opened", "trading"}: return True
        if status in {"closed", "close", "holiday", "weekend"}: return False
        return None
    ...
    async def _bulk_quotes(self, symbols):
        ...
        payload_timestamp, _ = self._find_timestamp(payload)

        def normalize_rows(rows_, fallback_timestamp=payload_timestamp):
            result_ = {}
            for row in rows_:
                ...
                if not symbol:
                    continue
                quote = self.quote_from_payload(row, symbol, fallback_timestamp)
        ...
        if len(normalized) > 3 and len(result) < max(2, int(len(normalized) * 0.20)):
            try:
                ...
                payload2_timestamp, _ = self._find_timestamp(payload2)
                result2 = normalize_rows(self._extract_rows(payload2), payload2_timestamp)
    ...
    async def market_summary(self):
        ...
        def pick(*keys, default=None):
            # Accept current and common alternate field names. Some API
            # revisions nest breadth/index values under market/index/breadth.
            containers = [data]
            for name in ("market", "index", "tasi", "breadth", "summary", "counts"):
        ...
        index_value = number(pick(
            "index_value", "tasi_value", "last", "last_price", "value", "index", "tasi"
        ), 0)
        change_percent = number(pick(
            "change_percent", "change_pct", "percent_change", "index_change_percent", "pct_change"
        ), 0)
        advancers = number(pick(
            "advancers", "advancing", "advancing_count", "gainers", "gainers_count", "up", "up_count"
        ), 0)
        decliners = number(pick(
            "decliners", "declining", "declining_count", "losers", "losers_count", "down", "down_count"
        ), 0)
        unchanged = number(pick(
            "unchanged", "unchanged_count", "flat", "flat_count", "unchanged_stocks"
        ), 0)
        ...
        return {
            ...
            "decliners": decliners,
            "unchanged": unchanged,
            "total_volume": number(pick("total_volume", "volume", "market_volume"), 0),
            "trading_value": number(pick("trading_value", "value_traded", "total_value", "market_value"), 0),
            "mood": pick("mood", "market_mood", "sentiment", default="") or "",
            "market_status_source": "TASILAB_MARKET_STATUS",
            "raw": data,
        }
    ...
    def stats(self):
        ...
        return {
            ...
            "daily_requests": (
                self._daily_request_attempts
            ),
            "daily_successful_requests": self._daily_successful_requests,
            "requests_last_minute": len([ts for ts in self._recent_request_times if ts >= time.monotonic() - 60.0]),
            "market_status_last_success": self._market_status_last_success,
            "market_status_last_error": self._market_status_last_error,
