...
def test_quote_accepts_symbol_mapping_and_missing_timestamp_is_rejected_by_freshness_gate():
    async def run():
        provider = TasilabProvider(make_settings())
        ...
        assert rows["1120"].updated_at is None
        assert rows["1120"].raw["_timestamp_source"] == "missing"
        await provider.close()
