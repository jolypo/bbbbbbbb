...
def test_live_intraday_without_mtf_is_diagnostic_not_hard_reject():
    a = assess_intraday(_trend_df(400), "BULLISH")
    assert a is not None
    assert not any("60 دقيقة" in x for x in a.hard_rejects)
    assert any("60 دقيقة" in x for x in a.reasons)
    assert any("اليومي" in x for x in a.reasons)
...
def test_regime_uses_breadth_not_only_index_change():
    ...
    assert classify_tasi({"change_percent": -0.2, "advancers": 40, "decliners": 170}) == "NEUTRAL"
    assert classify_tasi({"change_percent": -1.2, "advancers": 40, "decliners": 170}) == "BEARISH"
