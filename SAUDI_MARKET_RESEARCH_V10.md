# Saudi Market Research Basis — V10

V10 keeps the V8 Saudi-native calibration and adds evidence specifically supporting the new discovery, horizon and news-safety logic.
...
Engineering consequence: V10 retains the manual new-entry window 10:30–14:50 and does not fabricate an intraday closing price before a fresh post-close quote is available.
...
Engineering consequence: `LIMIT_UP`/`NEAR_LIMIT_UP` uses trustworthy provider/listing metadata when available; V10 never guesses “new listing ±30%” merely from the size of the price move.
...
Engineering consequence: V10 does not assume Free batch quotes. It keeps single-quote caching and adds a short 180-second Stage-1 discovery cache so back-to-back Intraday/Multi-Session searches can reuse the same delayed activity snapshot.
